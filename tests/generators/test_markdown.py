...
def test_use_table_for_fields_true(tmp_path: Path, simple_regmap: csr.Map) -> None:
    """Test that fields are rendered as a table when use_table_for_fields is True."""
    config = csr.MarkdownGenerator.Config(use_table_for_fields=True)
    gen = csr.MarkdownGenerator(
        label="test_md_gen_table_true_expects_table", register_map=simple_regmap, config=config, output_dir=tmp_path
    )
    generated_files = list(gen())
    output_file = generated_files[0]
    content = output_file.read_text()
    # Check for table headers for fields (should BE present)
    assert "| Name | Bits | Access " in content
    # Check for list-style field rendering (should NOT be present)
    assert "- **Bit" not in content


def test_use_table_for_fields_false(tmp_path: Path, simple_regmap: csr.Map) -> None:
    """Test that fields are rendered as a list when use_table_for_fields is False."""
    config = csr.MarkdownGenerator.Config(use_table_for_fields=False)
    gen = csr.MarkdownGenerator(
        label="test_md_gen_table_false_expects_list", register_map=simple_regmap, config=config, output_dir=tmp_path
    )
    generated_files = list(gen())
    output_file = generated_files[0]
    content = output_file.read_text()
    # Check for list-style field rendering (should BE present)
    assert "- **Bit" in content
    # Check for table headers for fields (should NOT be present)
    assert "| Name | Bits | Access " not in content
