...
from corsair import StrictEnum, StrictEnumMember

# All tests below can be used in smoke testing
pytestmark = pytest.mark.smoke


def build_member(**kwargs: Any) -> StrictEnumMember:
...
def build_enum(members: tuple[StrictEnumMember, ...], **kwargs: Any) -> StrictEnum:
    """Create default enum member."""
    defaults = {
        "name": "enum",
...
def test_invalid_value() -> None:
    ...
    with pytest.raises(ValidationError, match="Input should be greater than or equal to 0"):
        build_member(value=-1)


def test_width() -> None:
    """Test width of a member."""
    assert build_member(value=0b111).width == 3
    assert build_member(value=0).width == 1
    assert build_member(value=0b10101010).width == 8


def test_enum_width() -> None:
    """Test width of an enumeration."""
    assert build_enum((build_member(name="E1", value=0b1), build_member(name="E2", value=0b101))).width == 3
...
def test_enum_iter() -> None:
    ...
    enum = build_enum(
        (
            build_member(name="e1", value=1),
            build_member(name="e2", value=2),
            build_member(name="e3", value=3),
        )
    )
...
def test_enum_unique_values() -> None:
    ...
    members = (
        build_member(name="ENUM_VALUE_1", value=1),
        build_member(name="ENUM_VALUE_2", value=1),
    )
...
def test_enum_unique_names() -> None:
    ...
    members = (
        build_member(name="ENUM_VALUE_1", value=1),
        build_member(name="ENUM_VALUE_1", value=2),
    )
...
def test_enum_members_sorted() -> None:
    ...
    members = (
        build_member(name="ENUM_VALUE_2", value=2),
        build_member(name="ENUM_VALUE_1", value=1),
    )
